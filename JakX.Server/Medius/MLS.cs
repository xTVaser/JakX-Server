...
//                                             },
                            ...
                            case MediusAppPacketIds.GetAnnouncements:
                                {
                                    var msg = appMsg as MediusGetAnnouncementsRequest;
...
//                                             GenericField8 = x.GenericField8,
                                    ...
                                    {
                                        Console.WriteLine($"DEBUG: NO GAMES IN LIST");
                                        responses.Add(new RT_MSG_SERVER_APP()
                                        {
                            ...
                            case MediusAppPacketIds.GetLobbyPlayerNames:
                                {
                                    var msg = appMsg as MediusGetLobbyPlayerNamesRequest;

                                    responses.Add(new RT_MSG_SERVER_APP()
                                    {
                                        AppMessage = new MediusGetLobbyPlayerNamesResponse()
                                        {
                                            //MessageID = msg.MessageID,
                                            StatusCode = MediusCallbackStatus.MediusSuccess,
                                            AccountID = client.ClientObject.ClientAccount.AccountId,
                                            AccountName = client.ClientObject.ClientAccount.AccountName,
                                            EndOfList = true
                                        }
                                    });
                            ...
                            case MediusAppPacketIds.ChannelList_ExtraInfo:
                                {
                                    ...
                                    // JakX only uses this to connect to a non-game channel (lobby)
                            ...
                            case MediusAppPacketIds.GenericChatMessage:
                                {
                                    ...
                                    //copying structure from binary message
                                    
                                    var chatMessage = appMsg as MediusGenericChatMessage;
                                    ...
                                    switch (chatMessage.MessageType)
                                    {
                                        case MediusChatMessageType.Whisper:
                                            {
                                                var target = Program.GetClientByAccountId(chatMessage.TargetID);
                                                ...
                                                target?.AddLobbyMessage(new RT_MSG_SERVER_APP()
                                                {
                                                    ...
                                                    {
                                                        OriginatorAccountID = client.ClientObject.ClientAccount.AccountId,
                                                        OriginatorAccountName = client.ClientObject.ClientAccount.AccountName,
                                        ...
                                        case MediusChatMessageType.Broadcast:
                                            {
                                                client.ClientObject.CurrentChannel?.BroadcastChatMessage(client.ClientObject, chatMessage);
                                                break;
                                            }
                                        default:
                                            {
                                                Console.WriteLine($"Unhandled chat message type {chatMessage.MessageType}");
                                                break;
                                            }
                                    }
                                    break;
                            ...
                            case MediusAppPacketIds.AccountUpdatePassword:
                                {
                                    var msg = appMsg as MediusAccountUpdatePasswordRequest;

                                    responses.Add(new RT_MSG_SERVER_APP()
                                    {
                                        AppMessage = new MediusAccountUpdatePasswordResponse()
                                        {
                                            MessageID = msg.MessageID,
                                            StatusCode = MediusCallbackStatus.MediusFail
                                        }
